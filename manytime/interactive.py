...
class DecryptionsListBox(urwid.ListBox):
    ...
    REMOVE_KEYS = (Keys.BACKSPACE.value, Keys.DELETE.value)

    # Keys which are in urwid.command_map but we wish to use as input
    EXCLUDE_KEYS = (Keys.SPACE.value,)
    ...
    def _edit_decryption(self, letter: str) -> None:
        ...
        if letter == self.Keys.BACKSPACE.value:
    ...
    def keypress(self, size, key: str):
        ...
        if urwid.command_map[key] is None or key in self.EXCLUDE_KEYS:
            ...
            if key == self.Keys.BACKSPACE.value:
                key = self.Keys.LEFT.value
            elif key == self.Keys.DELETE.value:
                key = self.Keys.NO_KEY.value
            else:
                key = self.Keys.RIGHT.value
        elif key == self.Keys.ESCAPE_KEY.value:
